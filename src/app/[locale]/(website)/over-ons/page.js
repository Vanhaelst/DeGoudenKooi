...
async function getPage({ language, token }) {
  return fetchData(
    PageQuery({ page: "aboutUsEntries", language }),
    {
      revalidate: REVALIDATE,
      tags: [`page-aboutUsEntries`, `language-${language}`],
    },
    token,
  );
}
...
export async function generateMetadata({ params }) {
  const { page } = await fetchData(
    ...
    {
      revalidate: REVALIDATE,
      tags: [`metadata-aboutUsEntries`, `language-${params.locale}`],
    },
  );
  ...
  return {
    ...defaultMetadata,
    alternates: getAlternates({ locale: params.locale, path: "over-ons" }),
...
export default async function Home({ params, searchParams }) {
  ...
  const path = params.locale === "en" ? "about-us" : "over-ons";
  ...
  return (
    <ImageWrapper image={transparentImage}>
      <PageJsonLdScript
        ...
        page={currentPage}
        type="AboutPage"
        breadcrumbName={params.locale === "en" ? "Our story" : "Ons verhaal"}
      />
