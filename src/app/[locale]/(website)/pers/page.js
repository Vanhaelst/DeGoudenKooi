...
async function getPage({ language, token }) {
  return fetchData(
    PageQuery({ page: "persEntries", language }),
    {
      revalidate: REVALIDATE,
      tags: [`page-persEntries`, `language-${language}`],
    },
    token,
  );
}
...
export async function generateMetadata({ params }) {
  const { page } = await fetchData(
    SeoQuery({ page: "persEntries", language: params.locale }),
    {
      revalidate: REVALIDATE,
      tags: [`metadata-persEntries`, `language-${params.locale}`],
    },
  );
  ...
  return {
    ...defaultMetadata,
    alternates: getAlternates({ locale: params.locale, path: "pers" }),
...
export default async function Home({ params, searchParams }) {
  ...
  const path = params.locale === "en" ? "press" : "pers";
  ...
  return (
    <ImageWrapper image={transparentImage}>
      <PageJsonLdScript
        ...
        page={currentPage}
        type="CollectionPage"
        breadcrumbName={params.locale === "en" ? "Press" : "Pers"}
      />
